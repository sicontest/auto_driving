...
class DrivingController():
    ...
    def run(self):
        ...
        while self.game_playing(self.client):
            ...
            sensing_info.moving_forward = util.is_moving_forward(prev_state, car_state, self.way_points, lap_check_point_index)
            ...
            sensing_info.moving_angle = util.get_moving_angle(last_state, car_state, self.way_points, lap_check_point_index)

            current_lap = self.client.getAlgoUserAPI(self.player_name).ac_player_current_lap

            sensing_info.lap_progress = util.get_progress(car_state, self.way_points, lap_check_point_index, current_lap, 2)
            ...
            prev_time_millis = now_time_millis
    ...
    def getPlayerName(self, json_data):
        ...
        try:
            for key, value in json_data['Vehicles'].items():
                if self.player_name == "" and airsim.CarClient().isApiControlEnabled(key) == False:
                    self.player_name = key
                if key == player_name_check :
                    self.player_name = player_name_check
                    break
            if self.player_name != player_name_check :
    ...
    def load_track_info(self, client, json_pos):
        ...
        for x in range(0, len(way_points_raw)):
            if len(way_points_raw[x]):
                way_points_raw_trans = [way_points_raw[x][0]+(json_pos.x_val*-1),way_points_raw[x][1]+(json_pos.y_val*-1),0.0]
...
class DrivingUtil:
    ...
    def get_opponent_info(self, car_state, opponent_cars, way_points, check_point):
        ...
        for opp_car in opponent_cars:
            ...
            opp_prev, opp_next, opp_dist_from_prev, opp_to_middle = self.get_current_obstacle_info_full_scan(opp_car_pt, way_points)
